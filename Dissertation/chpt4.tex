...
\subsection{Introduction}
Simulated Annealing (SA) is a heuristic search technique propsed in the 80's by Kirkpatrick to solve combinatorial optimisation problems. The technique is based on a natural process which is known in thermodynamics as Annealing\footnote{Annealing will be discussed in the next sub section} \cite{SASingleMultiObj,TempCyclingSA}. 

SA has been applied to a wide range of optimisation problems due to is ability to find the global optimum. It has been proven that SA will find the global minimum in the solution space if certain criteria are met with regards to move movement generation \cite{ClusterSA}. The proof is beyond the scope of this disseration.

The SA algorithm is also very popular due to the basic structure of the algorithm being generic\cite{VariousCoolingSA}. Hence, applying the algorithm to other problems is relatively easy since only small changes are required. These changes usually need to applied to the Neighborhood selection scheme and the Cooling Schedule\cite{VariousCoolingSA}. Both of these concepts will be discussed in the next sub section.

The algorithm is also efficient with regards to CPU cycles when compared to the Genetic Algorithm because it only has to evaluate a certain number of moves each iteration, instead of a evaluating a whole population each iteration. Unlike Tabu Search the basic SA algorithm does not keep any memory and is therefore memory efficient, but in contrast suffers the risk of solution cyclicing.
