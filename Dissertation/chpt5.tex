...
The \gls{MMAS} algorithm as discussed also improves on the \gls{AS}. The global update rule used by \gls{AS} (see equation~\ref{eq:pheromoneupdate}) has a disadvantage in the sense that the search might concentrate too quickly on a particular good solution (the global best path)\cite{FundamentalSwarm}. \gls{MMAS} addresses this disadvantage by using the global update rule on the best path found per iteration\cite{FundamentalSwarm}. By using the update rule exclusively on the global best path allowed the swarm to converge to quickly on the global best path and limit exploration. By using the update rule on the best path per iteration exploration is increased and premature convergence is avoided to a degree\cite{FundamentalSwarm}.
...
In table~\ref{tab:ACO} the results obtained by Lune \emph{et al.}\cite{ACOvsEA} are presented. The values depicted in the table represent the amount of interference that will result if the plan is used in the network\cite{ACOvsEA}. By evaluating the results obtained, one can clearly see that the \gls{ACO} version that incorporates heuristic information to reinforce pheromone trails outperforms the version that does not \cite{ACOvsEA}. 
\begin{table}[H]
\centering
	\begin{tabular}{| c | c | c |}
	\hline
...
\label{tab:ACO}
\end{table}
This concludes the discussion on the \gls{ACO} algorithm. The following section discusses the Artificial Bee Algorithm.
...
\begin{figure}[H]
	\begin{center}
	\fbox{\includegraphics[width=4.5in,height=2.5in]{./pictures/geovelocity-new-text.pdf}}
...
\end{equation}
Where $D_{ab}$ is the value stored at row $a$, column $b$ of matrix $D$\cite{FundamentalSwarm}.
