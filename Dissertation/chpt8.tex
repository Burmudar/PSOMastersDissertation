...
The developed variants of the FAP PSO is compared against the algorithms that were applied the \gls{COST} 259 benchmarks as discussed in chapter \ref{chpt:heuristic}. Each algorithm against which the FAP PSO is compared against will now be listed. The name this chapter will use to refer to these algorithms are in bold face font. The section the algorithms were discussed in is as follows: Dynamic Tabu section~\ref{sec:tabuonfap}, k-thin FAP section~\ref{sec:saonfap}, GA~\ref{sec:gaonfap}.
...
The basis for how the results were produced has now been discussed. Furthermore, a layout for each section has been given. The next section presents the results obtained for the benchmark instance siemens 1.
